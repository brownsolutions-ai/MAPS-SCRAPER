import {regionOptions,regionName,leadRegion} from './regions.mjs';
...
export function showImport({state,toast,render,navigate}){
  ...
  let book=null,rows=[],headers=[],mapping={},filename='',country=state.country||'BR',region=state.region==='unassigned'?'':state.region||'';
  const countryOptions=()=>Object.entries(COUNTRIES).map(([code,name])=>`<option value="${code}" ${country===code?'selected':''}>${esc(name)}</option>`).join('');
  modal.innerHTML=`<div class="modal-header"><div><h2 id="modal-title">Importar planilha</h2><p>Excel, CSV ou TSV · até 25 MB</p></div><button class="icon-button" data-close aria-label="Fechar">${icon('close')}</button></div><div class="modal-body"><div class="field"><label for="import-country">País desta lista</label><select id="import-country">${countryOptions()}</select><small>Todos os leads desta planilha serão organizados na área escolhida.</small></div><div class="field"><label for="import-region">Estado / região para leads sem essa informação</label><select id="import-region">${regionOptions(country,region,'Usar apenas a coluna da planilha')}</select><small>A coluna Estado / região tem prioridade. Valores não reconhecidos ficam em Sem região definida para revisão.</small></div><div class="drop-zone" id="drop"><span class="upload-icon">${icon('upload')}</span><strong>Arraste sua planilha para cá</strong><p>O CRM mostra uma prévia antes de salvar qualquer empresa.</p><label for="file">Selecionar arquivo</label><input id="file" type="file" accept=".xlsx,.xls,.csv,.tsv" hidden></div></div><div class="modal-footer"><span class="small muted">Empresas repetidas preservam status, notas e follow-up.</span><button data-close>Cancelar</button></div>`;
  modal.showModal();
  const close=()=>modal.close();
  modal.querySelectorAll('[data-close]').forEach(b=>b.onclick=close);
  modal.oncancel=()=>{};
  const drop=modal.querySelector('#drop'),fileInput=modal.querySelector('#file');
  modal.querySelector('#import-country').onchange=e=>{country=e.target.value;region='';modal.querySelector('#import-region').innerHTML=regionOptions(country,'','Usar apenas a coluna da planilha');};
  modal.querySelector('#import-region').onchange=e=>{region=e.target.value;};
  ...
  function showPreview(){
    ...
    modal.querySelector('[data-confirm]').onclick=async e=>{const btn=e.currentTarget;btn.disabled=true;btn.innerHTML='<span class="loader"></span> Importando';try{const result=await importCompanies(state,plan,filename,country);state.country=country;state.region='';modal.close();navigate('all');toast(result.saved?`Planilha importada em ${COUNTRIES[country]} e salva no Supabase.`:'Planilha importada nesta demonstração. Conecte sua conta para salvar.');}catch(err){btn.disabled=false;btn.textContent='Tentar novamente';toast(err.message);}};
  }
}
