...
import {businessKey,mergeCompany,sameCompany} from './core.mjs';
...
export async function initialize(state,onRender,onToast){
  ...
  if(session){state.user=session.user;await loadAll(state);}
  db.auth.onAuthStateChange((event,session)=>{
    state.user=session?.user||null;
    if(event==='SIGNED_OUT'){state.demo=true;state.leads=[];state.events=[];state.imports=[];rerender();}
  });
}
...
export async function loadAll(state){
  ...
  state.demo=false;state.leads=companies.data||[];state.events=events.data||[];state.imports=imports.data||[];rerender();
}
...
export async function updateRecords(state,ids,patch){
  ...
  if(state.demo||!state.user){rerender();notify('Alteração feita na demonstração. Conecte sua conta para salvar.');return;}
  ...
  for(const id of ids){
    ...
    if(patch.status&&patch.status!==old.status)events.push({owner_id:state.user.id,company_id:id,event_type:'status_changed',from_status:old.status,to_status:patch.status,description:'Status atualizado'});
    if(Object.hasOwn(patch,'notes')&&patch.notes!==old.notes)events.push({owner_id:state.user.id,company_id:id,event_type:'note_updated',description:'Notas atualizadas'});
    if(Object.hasOwn(patch,'follow_up_at')&&patch.follow_up_at!==old.follow_up_at)events.push({owner_id:state.user.id,company_id:id,event_type:'follow_up_scheduled',description:patch.follow_up_at?'Próximo contato agendado':'Agendamento removido'});
  }
...
export async function importCompanies(state,plan,filename){
  ...
  const records=plan.leads.map(incoming=>{
    ...
    return {id:existing?.id,owner_id:state.user.id,business_key:businessKey(merged),name:merged.name,city:merged.city||null,state:merged.state||null,address:merged.address||null,phone:merged.phone||null,website:merged.website||null,rating:merged.rating,reviews:merged.reviews,category:merged.category||null,maps_url:merged.maps_url||null,place_id:merged.place_id||null,instagram:merged.instagram||null,email:merged.email||null,status:existing?.status||'new',notes:existing?.notes||'',favorite:existing?.favorite||false,follow_up_at:existing?.follow_up_at||null,updated_at:now};
  }).map(r=>Object.fromEntries(Object.entries(r).filter(([,v])=>v!==undefined)));
  for(let i=0;i<records.length;i+=300){const {error}=await db.from('companies').upsert(records.slice(i,i+300),{onConflict:'owner_id,business_key'});if(error)throw new Error(errMessage(error));}
  const batch={owner_id:state.user.id,filename,source_rows:plan.leads.length+plan.invalid+plan.duplicates,created_count:plan.created,updated_count:plan.updated,skipped_count:plan.invalid+plan.duplicates};
